class Controller {
  ...
  async getById(req, res) {
    const { id } = req.params;
    try {
      const responseRegister = await this.serviceEntity.getRegisterById(
        Number(id)
      );
  ...
  async create(req, res) {
    ...
    try {
      const newRegisterCreated =
        await this.serviceEntity.createNewRegister(dataToCreate);
  ...
  async update(req, res) {
    ...
    try {
      ...
      return res
        .status(200)
        .json({ message: "Success to update the register" });
    } catch (error) {
      console.log(error);
      return res.status(500).json({
        erro: error.message,
